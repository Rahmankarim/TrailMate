...
const sidebarItems = [
  ...
  { label: "Guides", icon: Compass, href: "/dashboard/company/guides", company: true },
  ...
  { label: "My Bookings", icon: Calendar, href: "/dashboard/user/bookings", user: true },
  { label: "Browse Packages", icon: Package, href: "/destinations", user: true },
  { label: "Chat", icon: MessageCircle, href: "/dashboard/user/chat", user: true },
...
export default function Sidebar({
  ...
  active = "Profile",
  onSelectAction,
}: { role?: "company" | "user"; active?: string; onSelectAction?: (label: string) => void }) {
  ...
  return (
    ...
    >
      ...
      <nav className="flex-1 flex flex-col gap-2 mt-4">
        {sidebarItems
          .filter((item) => {
            if (role === "company") return !item.user
            if (role === "user") return !item.company
            return true
          })
          .map((item) => (
            <Link
              key={item.label}
              href={item.href}
              className={`flex items-center gap-3 px-4 py-3 rounded-lg transition-all duration-200 hover:bg-white/10 ${active === item.label ? "bg-white/20" : ""}`}
